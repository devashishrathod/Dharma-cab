...
router.post("/create", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Error creating promo code" });
  }
});
...
router.get("/all", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Error fetching promo codes" });
  }
});
...
router.put("/update/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Error updating promo code" });
  }
});
...
router.delete("/delete/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Error deleting promo code" });
  }
});
...
router.post("/validate", accountMiddleware, async (req, res) => {
  ...
  if (!userId || !promoCode) {
    return res
      .status(400)
      .json({ success: false, message: "User ID and promo code are required" });
  }
  ...
  try {
    ...
    if (!code?.validUsers?.includes(userId) && code?.isOnlyForValidUser) {
      return res
        ...
        .json({
          success: false,
          message: "Promo code is not valid for this user",
        });
    }
...
router.post("/booking_price_calc", accountMiddleware, async (req, res) => {
  ...
  if (!userId || !rideType || !distance) {
    return res
      ...
      .json({
        success: false,
        message: "User ID, ride type, and distance are required",
      });
  }
  ...
  try {
    ...
    if (!vehiclePricingInfo) {
      return res
        ...
        .json({
          success: false,
          message: "Pricing info not found for this vehicle type",
        });
    }
    ...
    if (promoCode && promoCode?.trim()?.length() > 0) {
      ...
      if (
        baseAmount >= code?.minOrderAmount &&
        baseAmount <= code?.maxDiscountAmount &&
        code &&
        code?.isActive
      ) {
    ...
    if (
      ...
      return res.status(400).json({
        success: false,
        promoApplied: false,
        message: `Ride price must be between ₹${code.minOrderAmount} and ₹${code.maxDiscountAmount}`,
      });
    }
